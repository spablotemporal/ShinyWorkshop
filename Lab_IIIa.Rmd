---
title: "Lab_IIIa: Advanced visualization tools"
output:
  html_document:
    toc: true
    toc_float: true
...
We can also use polygon data with leaflet. We will first aggregate all the point layer information into a polygon data. To do this we will join the points with the polygon and then summarise the number of captures by polygon.
...
leaflet() %>% 
  addProviderTiles(providers$OpenStreetMap) %>% 
  addCircleMarkers(data = capturaSp, 
                   stroke = F,
                   radius = ~CAPTURADOS,
    popup = ~paste('Name:', NOM_LOC, 
                   "<br>Captures:", CAPTURADOS))
...
#                    ) %>%
#       addProviderTiles(providers$Stamen.TonerLabels, group="No Labels",
#                        options = providerTileOptions(maxZoom = 16)
#                    ) %>%
#   setView(lat = mean(data$lat, na.rm = T), lng = mean(data$lon, na.rm = T), zoom = 13) %>%
#   addCircleMarkers(stroke = F, radius = 10, opacity = 1.0, color = 'black', ~lon, ~lat, label = ~contact_person, popup = ~contact)
```
